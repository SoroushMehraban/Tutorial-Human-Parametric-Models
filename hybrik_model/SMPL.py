...
class SMPL_layer(nn.Module):
    ...
    def forward(self,
        ...
        twist_cos = torch.cos(twist_angle)
        twist_sin = torch.sin(twist_angle)
        phis = torch.cat([twist_cos, twist_sin], dim=2)
        ...
        output = edict(
            vertices=vertices, joints=joints, rot_mats=rot_mats, joints_from_verts=joints_from_verts_h36m, phis=phis, twist_angle=twist_angle)
        return output
